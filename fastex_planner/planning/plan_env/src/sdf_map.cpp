...
namespace fast_planner
{
  ...
  void SDFMap::initMap(ros::NodeHandle &nh)
  {
    ...
    // ESDF bounds are not symmetric around (0,0,0)
  ...
  void SDFMap::setCacheOccupancy(const int &adr, const int &occ)
  {
    ...
    if (md_->count_hit_[adr] == 0 && md_->count_miss_[adr] == 0)
    {
      ...
      // change_buffer_ collects all voxels within the range
      if (mp_->enable_change_detection_ &&
          mp_->change_colletion_mode_ == ALL_IN_PERCEPTION_RANGE &&
          isInBox(AddressToIndex(adr)))
      {
        change_mutex_.lock();
        md_->change_buffer_.emplace_back(adr);
        change_mutex_.unlock();
      }
    }
      ...
      const Eigen::Vector3d &sensor_pos)
  {
    ...
    {
      // extract each point in world coordinates
      auto &pt = points.points[i];
      pt_w << pt.x, pt.y, pt.z;
      int tmp_flag; // set tmp_flag to 1 if a substitute point was found
      // Set flag for projected point
      if (!isInMap(pt_w))
      {
        // Find closest point in map and set free
        // find the closest point to pt within map bounds
        pt_w = closetPointInMap(pt_w, sensor_pos);
        length = (pt_w - sensor_pos).norm();
        // find the closest point to pt within map bounds and max_ray_length_
      ...
      setCacheOccupancy(vox_adr, tmp_flag); // set occupancy at the point cloud location

      // determine the final update_min and update_max for the update region
      for (int k = 0; k < 3; ++k)
      {
        update_min[k] = min(update_min[k], pt_w[k]);
        update_max[k] = max(update_max[k], pt_w[k]);
      }
      // Raycasting between sensor origin and point
      // for the same inputPointCloud call, raycast each vox_adr only once
      ...
      caster_->nextId(idx);
      while (caster_->nextId(idx))            // returns false when reaching the endpoint
        setCacheOccupancy(toAddress(idx), 0); // set occupancy along the ray from sensor to point cloud point
    ...
    // continuously update log-odds values for each position
    while (!md_->cache_voxel_.empty())
    {
      ...
      // if hit count >= miss count, the position is more likely occupied; otherwise more likely free
      ...
      // update occupancy_buffer_ (skip already-occupied voxels)
      ...
      // change_buffer_ only collects voxels whose state has changed
      if (mp_->enable_change_detection_ && mp_->change_colletion_mode_ == ONLY_UPDATED &&
          isInBox(temp_idx) && getOccupancy(temp_idx) != last_state)
      {
        change_mutex_.lock();
        md_->change_buffer_.emplace_back(adr);
        change_mutex_.unlock();
  ...
  Eigen::Vector3d
  SDFMap::closetPointInMap(const Eigen::Vector3d &pt, const Eigen::Vector3d &sensor_pt)
  {
    Eigen::Vector3d diff = pt - sensor_pt;
    Eigen::Vector3d max_tc = mp_->map_max_boundary_ - sensor_pt;
    Eigen::Vector3d min_tc = mp_->map_min_boundary_ - sensor_pt;
    ...
    return sensor_pt + (min_t - 1e-3) * diff;
  }
  ...
  void SDFMap::clearAndInflateLocalMap()
  {
    ...
    // container inf_pts stores coordinates of points within the inflation range
    vector<Eigen::Vector3i> inf_pts(pow(2 * inf_step + 1, 3));
    // inf_pts.resize(4 * inf_step + 3);

    // triple nested loop over local bounding box, set inflate occupancy buffer to 0 (unoccupied)
    ...
    // second triple loop: convert 3D coordinates to index via toAddress(x, y, z),
    // then check if the occupancy buffer value exceeds mp_->min_occupancy_log_.
    // If so, call inflatePoint and store inflated point coordinates in inf_pts.
    // Finally, iterate inf_pts, convert each to index idx_inf, check if within valid map range,
    // if valid, set the corresponding inflate occupancy buffer value to 1 (occupied).
    for (int x = md_->local_bound_min_(0); x <= md_->local_bound_max_(0); ++x)
      for (int y = md_->local_bound_min_(1); y <= md_->local_bound_max_(1); ++y)
        for (int z = md_->local_bound_min_(2); z <= md_->local_bound_max_(2); ++z)
        {
    ...
    // add virtual ceiling to limit flight altitude: if mp_->virtual_ceil_height_ > -0.5, a virtual ceiling is enforced.
    // First, compute the virtual ceiling index ceil_id from the height coordinate.
    // Then, use a double nested loop to set the occupancy buffer to mp_->clamp_max_log_, clamping to max occupancy.
  ...
  bool SDFMap::isSDFMapGot()
  {
    return md_->sdf_map_got_;
  }
  ...
  /**
   * @brief Check if the straight path between two points is strictly collision-free.
   *
   * This function checks whether the straight path between the source and target points is strictly collision-free.
   * It uses the CollisionCheckResult structure but keeps the crosses_unknown flag always false.
   *
   * @param source The starting point of the path
   * @param target The ending point of the path
   * @return CollisionCheckResult A structure containing:
   *         - is_collision_free: true if the path is strictly collision-free, false otherwise
   *         - crosses_unknown: always false
   */
  SDFMap::CollisionCheckResult SDFMap::isStrictCollisionFreeStraight(const Eigen::Vector3d &source, const Eigen::Vector3d &target)
  {
    CollisionCheckResult result = {true, false};

    // Check if both source and target points are within the map boundaries
    if (isInBox(source) && isInBox(target))
    {
      // Check if source or target points are occupied
      if (getOccupancy(source) != FREE || getOccupancy(target) != FREE)
      {
        result.is_collision_free = false;
        return result;
      }
      else
      {
        // Initialize the collision checker with the source and target points
        std::unique_ptr<RayCaster> collision_check_caster = make_unique<RayCaster>();
        collision_check_caster->setParams(mp_->resolution_, mp_->map_origin_);

        collision_check_caster->input(source, target);
        Eigen::Vector3i start_idx = collision_check_caster->getStartIdx() + collision_check_caster->getOffset().cast<int>();
        Eigen::Vector3i end_idx = collision_check_caster->getEndIdx() + collision_check_caster->getOffset().cast<int>();
        int maxDist = (start_idx - end_idx).squaredNorm();

        // If the start and end indices are the same, the path is trivially collision-free
        ...
        else
        {
          ...
          {
            // If the current point is farther than the maximum distance, break the loop
            if ((current_idx - end_idx).squaredNorm() > maxDist)
            {
              break;
            }

            // If the current point is occupied or out of bounds, the path is not collision-free
  ...
  /**
   * @brief Check if the straight path between two points is collision-free and if it crosses unknown areas.
   *
   * This function checks whether the straight path between the source and target points is collision-free.
   * It also determines if the path crosses any unknown areas in the map.
   *
   * @param source The starting point of the path
   * @param target The ending point of the path
   * @return CollisionCheckResult A structure containing:
   *         - is_collision_free: true if the path is collision-free, false otherwise
   *         - crosses_unknown: true if the path crosses unknown areas, false otherwise
   */
  SDFMap::CollisionCheckResult SDFMap::isOptimisticCollisionFreeStraight(const Eigen::Vector3d &source, const Eigen::Vector3d &target)
  {
    CollisionCheckResult result = {true, false};
    ...
    {
      // Check if source or target points are occupied
      if ((getOccupancy(source) != FREE && getOccupancy(source) != UNKNOWN) ||
          (getOccupancy(target) != FREE && getOccupancy(target) != UNKNOWN))
      {
        result.is_collision_free = false;
        return result;
      }
      ...
      {
        // Initialize the collision checker with the source and target points
        unique_ptr<RayCaster> collision_check_caster = make_unique<RayCaster>();
        collision_check_caster->setParams(mp_->resolution_, mp_->map_origin_);
        ...
        else
        {
          ...
          while (collision_check_caster->nextId(current_idx))
          {
            ...
            int occupancy = getOccupancy(current_idx);
            // If the current point is occupied or out of bounds, the path is not collision-free
            if ((occupancy != FREE && occupancy != UNKNOWN) || !isInBox(current_idx))
            {
              result.is_collision_free = false;
              return result;
            }

            // If the current point is unknown, set the crosses_unknown flag
            if (occupancy == UNKNOWN)
            {
              result.crosses_unknown = true;
            }
          }
  ...
  SDFMap::CollisionCheckResult SDFMap::isStrictInflatedCollisionFreeStraight(const Eigen::Vector3d &source, const Eigen::Vector3d &target)
  {
    CollisionCheckResult result = {true, false};
    ...
    {
      // Check if source or target points are occupied
      if (getInflateOccupancy(source) != 0 || getInflateOccupancy(target) != 0 ||
          getOccupancy(source) != FREE || getOccupancy(target) != FREE)
      {
        result.is_collision_free = false;
        return result;
      }
      else
      {
        ...
        else
        {
          ...
          while (collision_check_caster->nextId(current_idx))
          {
            ...
            // If the current point is occupied or out of bounds, the path is not collision-free
            if (getInflateOccupancy(current_idx) != 0 ||
                getOccupancy(current_idx) != FREE || !isInBox(current_idx))
            {
              result.is_collision_free = false;
              return result;
            }
  ...
  SDFMap::CollisionCheckResult SDFMap::isOptimisticInflatedCollisionFreeStraight(const Eigen::Vector3d &source, const Eigen::Vector3d &target)
  {
    CollisionCheckResult result = {true, false};
    ...
    if (isInBox(source) && isInBox(target))
    {
      // Check if source or target points are occupied
      if (getInflateOccupancy(source) != 0 || getInflateOccupancy(target) != 0 ||
          (getOccupancy(source) != FREE && getOccupancy(source) != UNKNOWN) ||
          (getOccupancy(target) != FREE && getOccupancy(target) != UNKNOWN))
      {
        result.is_collision_free = false;
        return result;
      ...
      else
      {
        ...
        else
        {
          ...
          while (collision_check_caster->nextId(current_idx))
          {
            ...
            int occupancy_inf = getInflateOccupancy(current_idx);
            int occupancy = getOccupancy(current_idx);
            // If the current point is occupied or out of bounds, the path is not collision-free
            if (occupancy_inf != 0 || (occupancy != FREE && occupancy != UNKNOWN) || !isInBox(current_idx))
            {
              result.is_collision_free = false;
              return result;
            }
...
/**
 * @brief Find the nearest free cell using BFS in the SDF map within a search box.
 *
 * @param search_box_size The size of the search box
 * @param center_pos The center position of the search box
 * @param nearest_valid_pos The nearest valid position (output)
 * @return true If a free cell is found
 * @return false If no free cell is found within the search box
 */
bool fast_planner::SDFMap::bfsNearestFree(const Eigen::Vector3d &search_box_size,
                                          const Eigen::Vector3d &center_pos,
                                          Eigen::Vector3d &nearest_valid_pos)
{
    Eigen::Vector3i search_box_min_ijk, search_box_max_ijk;
    posToIndex(center_pos - search_box_size / 2.0, search_box_min_ijk);
    posToIndex(center_pos + search_box_size / 2.0, search_box_max_ijk);

    Eigen::Vector3i start_ijk;
    posToIndex(center_pos, start_ijk);

    std::queue<Eigen::Vector3i> q;
    std::unordered_set<int> visited;
    std::vector<Eigen::Vector3i> candidates;

    double sdf_resl = getResolution();

    q.push(start_ijk);
    if (isInMap(start_ijk))
        visited.insert(toAddress(start_ijk));

    Eigen::Vector3i cur_ijk;
    ...
    while (!q.empty())
    {
        ...
        if (isInflatedFree(cur_ijk))
        {
            candidates.push_back(cur_ijk);
            indexToPos(cur_ijk, nearest_valid_pos);

            if (!isNeighborOccupied(nearest_valid_pos, sdf_resl))
                return true;
        }
        ...
        for (int i = -1; i <= 1; ++i)
            for (int j = -1; j <= 1; ++j)
                for (int k = -1; k <= 1; ++k)
                {
                    ...
                    Eigen::Vector3i nbr = cur_ijk + Eigen::Vector3i(i, j, k);

                    // Check bounds before computing address
                    if (!isInMap(nbr) || !isInBox(nbr))
                        continue;

                    int addr = toAddress(nbr);
                    if (visited.count(addr))
                        continue;

                    if (isInflatedUnknown(nbr))
                        continue;

                    // Check if within search box
                    bool in_search_box = true;
                    for (int d = 0; d < 3; ++d)
                    {
                        if (nbr[d] < search_box_min_ijk[d] || nbr[d] > search_box_max_ijk[d])
                        {
                            in_search_box = false;
                            break;
                        }
    ...
    if (!candidates.empty())
    {
        indexToPos(candidates.back(), nearest_valid_pos);
        return true;
    }
