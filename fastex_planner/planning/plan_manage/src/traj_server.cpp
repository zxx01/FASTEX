...
vector<Eigen::Vector3d> traj_cmd_, traj_real_, sparse_traj_cmd_;
                          ...
                          int id)
{
  ...
  auto scale = vis_utils::marker_utils::makeScale(resolution, 0.0, 0.0);

  // DELETE old marker
  auto mk_del = vis_utils::marker_utils::makeMarker(
      "world", "", id, visualization_msgs::Marker::LINE_STRIP, scale, ros_color,
...
void visCallback(const ros::TimerEvent &e)
{
  ...
  displayTrajWithColor(sparse_traj_cmd_, 0.2, Eigen::Vector4d(0, 0, 1, 1), pub_traj_id_);
                     ...
                     bspline::BsplineProcess::Response &resp)
{
  ...
  start_time_ = bspline_req.start_time; // max(preset trajectory start time, actual time when planning finishes)
  traj_id_ = bspline_req.traj_id;

  // store all trajectory data for one execution (position, velocity, acceleration, yaw, yaw rate, yaw acceleration)
  traj_.clear();
...
void bsplineCallback(const bspline::BsplineConstPtr &msg)
{
  ...
  start_time_ = msg->start_time; // max(preset trajectory start time, actual time when planning finishes)
