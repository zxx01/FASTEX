...
inline AABB::AABB(unsigned int dimension)
{
    assert(dimension >= 2);
...
inline AABB::AABB(const std::vector<double>& lowerBound_, const std::vector<double>& upperBound_)
...
inline double AABB::computeSurfaceArea() const
{
...
inline double AABB::getSurfaceArea() const { return surfaceArea; }

inline void AABB::merge(const AABB& aabb1, const AABB& aabb2)
{
...
inline bool AABB::overlaps(const AABB& aabb, bool touchIsOverlap) const
{
    assert(aabb.lowerBound.size() == lowerBound.size());
...
inline std::vector<double> AABB::computeCentre()
{
...
inline void AABB::setDimension(unsigned int dimension)
{
    assert(dimension >= 2);
...
inline bool Node::isLeaf() const { return (left == NULL_NODE); }

inline Tree::Tree(unsigned int dimension_, double skinThickness_, unsigned int nParticles,
...
inline Tree::Tree(unsigned int dimension_, double skinThickness_,
                  const std::vector<bool>& periodicity_, const std::vector<double>& boxSize_,
                  unsigned int nParticles, bool touchIsOverlap_)
    : dimension(dimension_), skinThickness(skinThickness_), periodicity(periodicity_),
...
inline void Tree::setPeriodicity(const std::vector<bool>& periodicity_)
{
    periodicity = periodicity_;
}

inline void Tree::setBoxSize(const std::vector<double>& boxSize_) { boxSize = boxSize_; }

inline unsigned int Tree::allocateNode()
{
...
inline void Tree::freeNode(unsigned int node)
{
...
inline void Tree::insertParticle(unsigned int particle, std::vector<double>& position,
                                 double radius)
{
...
inline void Tree::insertParticle(unsigned int particle, std::vector<double>& lowerBound,
                                 std::vector<double>& upperBound)
{
    // Make sure the particle doesn't already exist.
    if (particleMap.count(particle) != 0)
...
inline unsigned int Tree::nParticles() { return particleMap.size(); }

inline void Tree::removeParticle(unsigned int particle)
{
    // Map iterator.
...
inline bool Tree::updateParticle(unsigned int particle, std::vector<double>& position,
...
inline bool Tree::updateParticle(unsigned int particle, std::vector<double>& lowerBound,
...
inline std::vector<unsigned int> Tree::query(unsigned int particle)
{
...
inline std::vector<unsigned int> Tree::query(unsigned int particle, const AABB& aabb)
{
...
inline std::vector<unsigned int> Tree::query(const AABB& aabb)
{
...
inline const AABB& Tree::getAABB(unsigned int particle)
{
    return nodes[particleMap[particle]].aabb;
}

inline void Tree::insertLeaf(unsigned int leaf)
{
...
inline void Tree::removeLeaf(unsigned int leaf)
{
    if (leaf == root)
    {
        root = NULL_NODE;
...
inline unsigned int Tree::balance(unsigned int node)
{
...
inline unsigned int Tree::computeHeight() const { return computeHeight(root); }

inline unsigned int Tree::computeHeight(unsigned int node) const
{
    assert(node < nodeCapacity);
...
inline unsigned int Tree::getHeight() const
{
    if (root == NULL_NODE)
        return 0;
...
inline unsigned int Tree::getNodeCount() const { return nodeCount; }

inline unsigned int Tree::computeMaximumBalance() const
{
...
inline double Tree::computeSurfaceAreaRatio() const
{
    if (root == NULL_NODE)
        return 0.0;
...
inline void Tree::validateStructure(unsigned int node) const
{
    if (node == NULL_NODE)
        return;
...
inline void Tree::validateMetrics(unsigned int node) const
{
    if (node == NULL_NODE)
        return;
...
inline void Tree::periodicBoundaries(std::vector<double>& position)
{
    for (unsigned int i = 0; i < dimension; i++)
    {
        if (position[i] < 0)
...
inline bool Tree::minimumImage(std::vector<double>& separation, std::vector<double>& shift)
{
