...
template <typename PointType> int KD_TREE<PointType>::size()
{
    ...
    if (Rebuild_Ptr == nullptr || *Rebuild_Ptr != Root_Node)
    {
        if (Root_Node != nullptr)
        {
            ...
            if (s == 0)
                std::cerr << "(11)Error: Tree size is zero\n";
        }
        else
        {
            s = 0;
    ...
    else
    {
        if (!pthread_mutex_trylock(&working_flag_mutex))
        {
            ...
            if (s == 0)
                std::cerr << "(01)Error: Tree size is zero\n";
        }
        else
        {
            s = Treesize_tmp;
            if (s == 0)
                std::cerr << "(00)Error: Tree size is zero\n";
        }
    }
    return s;
}
...
void KD_TREE<PointType>::Delete_by_point(KD_TREE_NODE** root, PointType point, bool allow_rebuild)
{
    ...
    {
        // if the rebuilt tree root pointer from thread 2 is null, or does not point to either child
        // of the current root
        if ((Rebuild_Ptr == nullptr) ||
            ((*root)->left_son_ptr != *Rebuild_Ptr && (*root)->right_son_ptr != *Rebuild_Ptr))
        {
            Delete_by_point(&(*root)->left_son_ptr, point, allow_rebuild);
            Delete_by_point(&(*root)->right_son_ptr, point, allow_rebuild);
        }
        // if the rebuilt tree root pointer from thread 2 is not null and points to one of the
        // current root children
        else
        {
            // if thread 2's rebuilt root points to the left child
            ...
            // if thread 2's rebuilt root points to the right child
            else
            {
                Delete_by_point(&(*root)->left_son_ptr, point, allow_rebuild);
